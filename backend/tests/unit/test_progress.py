...
from app.services.progress_service import ProgressService, XP_BASE, LEVEL_THRESHOLDS
...
def test_xp_values_are_correct():
    ...
    assert XP_BASE["reading"] == 5
    assert XP_BASE["listening"] == 10
    assert XP_BASE["writing"] == 20
    assert XP_BASE["speaking"] == 8


@pytest.mark.asyncio
async def test_record_progress_awards_correct_xp_for_reading():
    """Req 10.1: completing a reading module awards XP based on score multiplier."""
    mock_db = AsyncMock()
    service = ProgressService(mock_db)
    user = _make_user(xp_total=0)
    record = _make_progress_record("reading", 100) # perfect score -> 100% of 5 = 5 XP

    with patch.object(service.repo, "create_lesson_progress", return_value=record):
        with patch.object(service, "_add_xp", return_value=5) as mock_xp:
            with patch.object(service, "check_level_up", return_value=None):
                result = await service.record_lesson_progress(
                    user.id,
                    LessonProgressRequest(
                        lesson_id=uuid.uuid4(), module_type="reading", score=100
                    ),
                )

    mock_xp.assert_called_once_with(user.id, 5)
    assert result.xp_awarded == 5


@pytest.mark.asyncio
async def test_record_progress_awards_correct_xp_for_speaking():
    """Req 10.1: completing a speaking module awards full 8 XP regardless of score."""
    mock_db = AsyncMock()
    service = ProgressService(mock_db)
    user = _make_user(xp_total=0)
    ...
    with patch.object(service.repo, "create_lesson_progress", return_value=record):
        with patch.object(service, "_add_xp", return_value=8) as mock_xp:
            with patch.object(service, "check_level_up", return_value=None):
                result = await service.record_lesson_progress(
                    user.id,
                    LessonProgressRequest(
    ...
    mock_xp.assert_called_once_with(user.id, 8)
...
def test_level_thresholds_are_correct():
    """Req 10.3: thresholds must be 300 XP for B2 and 1200 XP for C1."""
    threshold_map = {level: xp for xp, level in LEVEL_THRESHOLDS}
    assert threshold_map["B2"] == 300
    assert threshold_map["C1"] == 1200


@pytest.mark.asyncio
async def test_check_level_up_b1_to_b2_at_threshold():
    """Req 10.2: user at exactly 300 XP advances from B1 to B2."""
    mock_db = AsyncMock()
    service = ProgressService(mock_db)

    user = _make_user(level_band="B1", xp_total=300)
    ...
    await service.check_level_up(user.id, 300)
...
async def test_check_level_up_b2_to_c1_at_threshold():
    """Req 10.2: user at exactly 1200 XP advances from B2 to C1."""
    mock_db = AsyncMock()
    service = ProgressService(mock_db)

    user = _make_user(level_band="B2", xp_total=1200)
    ...
    await service.check_level_up(user.id, 1200)
...
async def test_check_level_up_no_change_below_threshold():
    """No level-up when XP is below 300."""
    mock_db = AsyncMock()
    service = ProgressService(mock_db)

    user = _make_user(level_band="B1", xp_total=299)
    ...
    await service.check_level_up(user.id, 299)
